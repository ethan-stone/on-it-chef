...
import { z } from "zod";
import { RecipeVersion } from "../repos/recipe-versions";
...
export type RecipeGeneratorUsageMetadata = {
  model: string;
  durationMs: number;
  inputTokens: number;
  outputTokens: number;
...
export type RecipeGeneratorResult<T> = {
  text: string;
  content: T;
  usageMetadata: RecipeGeneratorUsageMetadata;
};

export class RecipeGenerator {
  constructor(private readonly googleGenAI: GoogleGenAI) {}

  private parseResult(result: GenerateContentResponse) {
    ...
    return AIRecipeResponse.parse(JSON.parse(jsonText));
  }

  async create(userPrompt: string, dietaryRestrictions?: string | null) {
    const prompt = `${CREATE_RECIPE_SYSTEM_PROMPT}${
      dietaryRestrictions
